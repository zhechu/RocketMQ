...
public class ExpressionMessageFilter implements MessageFilter {
     ...
     * 如果订阅消息的TAGhashcodes集合中包含消息的tagsCode，返回true。
     * 基于TAG模式，根据ConsumeQueue进行消息过滤时只对比tag的hashcode，所以基于TAG模式消息过滤，还需要在消息消费端对消息tag进行精确匹配
     ...
     * 如果是类过滤模式，返回true；
     * 如果是TAG模式，返回true。
     * 该方法主要是为表达式模式SQL92服务的，根据消息属性实现类似于数据库SQLwhere条件过滤方式
