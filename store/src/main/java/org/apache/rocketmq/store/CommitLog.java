...
public class CommitLog {
    ...
    protected static int calMsgLength(int bodyLength, int topicLength, int propertiesLength) {
        final int msgLen = 4 //TOTALSIZE 该消息条目总长度
            ...
            + 4 //BODYCRC 消息体crc校验码
            + 4 //QUEUEID 消息消费队列ID
            + 4 //FLAG 消息FLAG，RocketMQ不做处理，供应用程序使用
            + 8 //QUEUEOFFSET 消息在消息消费队列的偏移量
            + 8 //PHYSICALOFFSET 消息物理偏移量
            ...
            + 1 + topicLength //TOPIC 主题存储长度，1字节，表示主题名称不能超过255个字符
            + 2 + (propertiesLength > 0 ? propertiesLength : 0) //propertiesLength 消息属性长度，2字节，表示消息属性长度不能超过65536个字符
            + 0;
        return msgLen;
    }
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                // 若客户端上传的消息延迟级别大于服务端最大延迟级别，则修正为服务端最大延迟级别
                if (msg.getDelayTimeLevel() > this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel()) {
                    msg.setDelayTimeLevel(this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel());
                }

                // 将消息的原主题名称与原消息队列ID存入消息属性中，用延迟消息主题SCHEDULE_TOPIC、消息队列ID更新原先消息的主题与队列
        ...
        // MappedFile对${ROCKET_HOME}/store/commitlog文件夹下一个个的文件
        ...
        try {
            ...
            if (null == mappedFile || mappedFile.isFull()) {
                // 如果mappedFile为空，表明${ROCKET_HOME}/store/commitlog目录下不存在任何文件，
    ...
    /**
     * RocketMQ的存储与读写是基于JDKNIO的内存映射机制（MappedByteBuffer）的，消息存储时首先将消息追加到内存，
     * 再根据配置的刷盘策略在不同时间进行刷写磁盘。如果是同步刷盘，消息追加到内存后，将同步调用MappedByteBuffer的force()方法；
    ...
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 如果距上次提交间隔超过commitDataThoroughInterval，则本次提交忽略commitCommitLogLeastPages参数，
                ...
                try {
                    // 执行提交操作，将待提交数据提交到物理文件的内存映射内存区，如果返回false，并不是代表提交失败，而是只提交了一部分数据，
                    // 唤醒刷盘线程执行刷盘操作。该线程每完成一次提交动作，将等待200ms再继续执行下一次提交任务
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 如果距上次提交间隔超过flushPhysicQueueThoroughInterval，则本次刷盘任务将忽略flushPhysicQueueLeastPages，
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        /**
         * 由于避免同步刷盘消费任务与其他消息生产者提交任务直接的锁竞争，GroupCommitService提供读容器与写容器，
        ...
        private void doCommit() {
            synchronized (this.requestsRead) {
                if (!this.requestsRead.isEmpty()) {
                    ...
                    // 处理完所有同步刷盘任务后，更新刷盘检测点StoreCheckpoint中的physicMsgTimestamp，
