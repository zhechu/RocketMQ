...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 创建MQClientInstance实例。整个JVM实例中只存在一个MQClientManager实例，维护一个MQClientInstance缓存
                // 表ConcurrentMap<String/*clientId*/,MQClientInstance>factoryTable=newConcurrentHashMap<String,MQClientInstance>(),
    ...
    private SendResult sendDefaultImpl(
        ...
        // 确保生产者处于运行状态
        this.makeSureStateOK();
        // 验证消息是否符合相应的规范，具体的规范要求是主题名称、消息体不能为空、消息长度不能等于0
        // 且默认不能超过允许发送消息的最大长度4M（maxMessageSize=1024*1024*4）
    ...
    /**
     * 如果生产者中缓存了topic的路由信息，如果该路由信息中包含了消息队列，则直接返回该路由信息，如果没有缓存或没有包含消息队列，
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        switch (sendResult.getSendStatus()) {
            // 如果消息发送成功，则执行TransactionListener#executeLocalTransaction方法，
            // 该方法的职责是记录事务消息的本地事务状态，
            // 例如可以通过将消息唯一ID存储在数据中，并且该方法与业务代码处于同一个事务，与业务事务要么一起成功，要么一起失败。
            // 这里是事务消息设计的关键理念之一，为后续的事务状态回查提供唯一依据
            case SEND_OK: {
                try {
     ...
     * 其关键就是根据本地执行事务的状态分别发送提交、回滚或不作为的命令
     * @param sendResult
